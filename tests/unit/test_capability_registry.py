...
def test_builtin_registry_is_versioned_and_contains_only_approved_implementations():
    ...
    assert first.catalog_version == "0.36.0"
    assert first.catalog_digest == second.catalog_digest
    assert len(first.capabilities) == 52
    ...
    fastapi_command = first.definition_for_implementation("SEC-FASTAPI-COMMAND-INJECTION-001")
    assert fastapi_command.capability_id == "control.native.fastapi-direct-command-injection"
    assert fastapi_command.frameworks == frozenset({"FastAPI"})
    assert fastapi_command.languages == frozenset({"Python"})
    python_jwt = first.definition_for_implementation("SEC-JWT-PYTHON-VERIFY-001")
    assert python_jwt.capability_id == "control.native.python-jwt-signature-verification"
    assert python_jwt.languages == frozenset({"Python"})
    assert python_jwt.frameworks == frozenset()
