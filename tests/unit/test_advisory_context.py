...
def test_range_context_reads_target_git_tree_not_current_worktree(tmp_path: Path):
    ...
    (repository / "src" / "a.py").write_text(target, encoding="utf-8")
...
def test_rendered_manifest_is_content_free_but_inspectable(tmp_path: Path):
    ...
    (repository / "src" / "a.py").write_text(secret_like_source, encoding="utf-8")
...
def test_workspace_symlink_is_excluded_without_reading_target(tmp_path: Path):
    repository = _repository(tmp_path)
