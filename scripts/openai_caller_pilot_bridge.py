...
DEFAULT_CASES_PATH = "fixtures/caller-pilot-v2/cases.json"
        ...
        "input": [
            {
                ...
                "content": (
                    ...
                    "through a parameterized interface, coercing a value into a bounded range "
                    "(such as int(x) or max/min numeric clamping), or comparing secrets in constant time, "
                    "is safe and is not a finding even when a caller passes untrusted input to it. "
                    "Converting an input to an integer or applying range limits is standard numeric coercion, not a bug. "
                    "A claim must select one primary evidence ID that appears in this payload; an optional secondary "
                    "evidence ID may be selected when both helper and caller evidence are material. "
                    "A claim that cannot cite supplied evidence must be omitted rather than weakened. "
                    "\n\n"
                    ...
                    "When reviewing caller observations, evaluate ONLY the specific call site and "
                    "data flow of the queried symbol. Do not inspect or report findings for "
                    "unrelated functions or code in the caller file that do not invoke the queried symbol. "
                    "\n\n"
                    "Expansion discipline. When the initial evidence alone concretely supports the "
                    "claim (for example, a self-contained defect in the helper such as a comparison "
                    "against itself, an inverted check like != or reversed inequality, or an unconditional grant), "
                    "always locate the finding directly in that helper function and return FINAL immediately without expanding. "
                    "Request FIND_CALLERS only when the helper itself is neutral about the claimed property and caller "
                    "evidence is required to decide the claim. "
                    "\n\n"
