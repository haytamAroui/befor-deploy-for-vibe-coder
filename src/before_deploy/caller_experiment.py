...
class FindCallersIndex:
    ...
    def __init__(self, repository: Path, *, budget: CallerBudget | None = None) -> None:
        ...
        for source in inventory.files:
            if len(files) >= self.budget.max_index_files:
                break
            relative = source.relative_to(inventory.root)
            if _path_has_symlink(inventory.root, relative):
                continue
            try:
            ...
            if total_bytes + len(raw) > self.budget.max_index_bytes:
                continue
            files.append((relative.as_posix(), tuple(text.splitlines())))
            total_bytes += len(raw)
    ...
    def find_callers(self, *, call_id: str, symbol: str) -> CallerObservation:
        ...
        for path, lines in self.files:
            for line_number, text in enumerate(lines, start=1):
                ...
                if any(pattern.search(text) for pattern in definition_patterns):
                    continue
                start = max(1, line_number - self.budget.caller_context_lines)
                end = min(len(lines), line_number + self.budget.caller_context_lines)
                snippet = [
        ...
        payload: Mapping[str, object] = {
            ...
            "symbol": symbol,
            "semantics": "lexical_static_call_sites_with_bounded_nearby_context_not_runtime_reachability",
            "call_sites": selected,
        ...
        try:
            ...
            for step in range(1, self.budget.max_steps + 1):
                ...
                if len(observations) >= self.budget.max_tool_calls:
                    ...
                    break
                observation = index.find_callers(
                    call_id=turn.call_id or "",
                    symbol=turn.symbol or "",
                )
...
def _normalize_claims(
    ...
    for draft in drafts:
        ...
        path = _safe_relative_path(draft.path) if draft.path else None
...
def _safe_relative_path(value: str) -> str:
    if not isinstance(value, str) or not value.strip():
        raise ValueError("Caller path must be non-empty")
    normalized = value.strip().replace("\\", "/")
    candidate = PurePosixPath(normalized)
    if candidate.is_absolute() or ".." in candidate.parts:
        raise ValueError("Caller path must be repository-relative")
    if len(normalized) >= 3 and normalized[1:3] == ":/":
        raise ValueError("Caller path must be repository-relative")
    if any(part in {"", "."} for part in candidate.parts):
        raise ValueError("Caller path must be canonical")
...
def _lines(
    ...
    if start is None:
        if end is not None:
            raise ValueError("Caller claim end_line requires start_line")
        return None, None
