...
| **Native controls** | High-confidence secret patterns, selected Python SQL interpolation, FastAPI mutating-route authentication declarations, Python debug/CORS patterns, Next.js public-environment, session-cookie, static-CORS, and Server Action local-guard-marker checks, GitHub Actions hardening, dependency lockfile presence, an exact offline Go vulnerability snapshot check, and a release SBOM check. |
