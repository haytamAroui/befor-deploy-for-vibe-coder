...
`find_callers` is a bounded deterministic lexical observation over the canonical repository inventory. It returns repository-relative call-site locations and bounded source-line snippets. It explicitly does **not** claim runtime reachability, type resolution, semantic equivalence, or interprocedural dataflow.
